module ApplicationHelper
  def index_links
    out = ''
    if user_signed_in?
      out += "<p>#{link_to(current_user.name, edit_user_registration_path, class: "button is-info")}</p>"
      out += "<p>#{link_to("Log Out", destroy_user_session_path, method: :delete, class:"button is-info")}</p>"
      out += "<p>#{link_to("New Post", new_post_path, class: "button is-info is-inverted")}</p>"
    else
